...
{
	cout << "GTEC engine v1.8.1..ok" << endl;
	cout << "(c) Andreas Hoppe" << endl;
	
	// set graphics context
	pgame->SetGraphics (graphics);
	
	 // Handling of loading screen
	CTexture LoadingScreen;
	if (LoadingScreen.LoadTexture("images/LoadingScreen.bmp"))
	{
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);	
		LoadingScreen.Draw(Width/2,Height/2);
		SDL_GL_SwapBuffers();
	}
